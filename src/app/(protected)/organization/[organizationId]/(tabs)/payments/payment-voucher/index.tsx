...
export default function PaymentVoucherIndex() {
    ...
    async (pageToFetch: number, isRefreshing = false) => {
      ...
      try {
        ...
        const res = await axios.get(
          `${API_BASE}/api/${organizationId}/payment?page=${pageToFetch}&limit=${PAGE_SIZE}`,
        ...
        setData((prev) =>
          isRefreshing ? newPayments : [...prev, ...newPayments]
        );

        // If we get fewer items than PAGE_SIZE, we've hit the end
  ...
  const handleDelete = (id: string) => {
    Alert.alert("Delete Voucher", "Are you sure you want to delete this?", [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            const token = await getToken();
            await axios.delete(
              `${API_BASE}/api/${organizationId}/payment/${id}`,
              {
                headers: { Authorization: `Bearer ${token}` },
              }
            );
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <View>
          ...
          <Text style={styles.headerTitle}>Receipt Vouchers</Text>
        </View>
        ...
        <TouchableOpacity
          ...
          onPress={() =>
            router.push(
              `/organization/${organizationId}/payments/payment-voucher/new`
            )
          }
        >
          ...
          <View style={styles.card}>
            ...
            <View style={styles.actions}>
              <TouchableOpacity
                onPress={() =>
                  router.push(
                    `/organization/${organizationId}/payments/payment-voucher/${item.id}`
                  )
                }
...
const styles = StyleSheet.create({
  ...
  amountText: { fontSize: 17, fontWeight: "800", color: "#059669" },
  cardBody: {
    flexDirection: "row",
    justifyContent: "space-between",
    marginTop: 8,
